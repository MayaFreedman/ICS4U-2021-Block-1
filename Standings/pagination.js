...
function createStandings() {
    display = userRecords.slice(((pageNum * 10) - 10), (pageNum * 10));
...
function createRow(user) {
    ...
    newCell.appendChild(document.createTextNode(user.id));
...
function createButtons() {
    if (pageNum < 8) {
        let currentPage = document.querySelector('#lowestPage');
        currentPage.innerHTML = pageNum;
        document.querySelector('#middlePage').innerHTML = pageNum + 1;
        document.querySelector('#higherPage').innerHTML = pageNum + 2;
        document.querySelector('#otherPages').innerHTML = '...';
    }
    if (pageNum >= 8) {
        document.querySelector('#lowestPage').innerHTML = '8';
        document.querySelector('#middlePage').innerHTML = '9';
        document.querySelector('#higherPage').innerHTML = '10';
        document.querySelector('#otherPages').innerHTML = '11';
    }
    for (let i = 0; i < buttons.length; i++) {
        buttons[i].style.outline = "none";

        if (buttons[i].innerHTML == pageNum)
...
function goToPage(element) {
    if (element.innerHTML != "...") {
        pageNum = parseInt(element.innerHTML);
        createStandings();
        createButtons();
    }
}
